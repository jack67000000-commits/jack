...
export default function GamesPage(){
  ...
  return <div className="adminShell"><AdminSidebar active="games"/><main className="content gamesContent">
    ...
    <section className="catalog"><div className="catalogTop"><label>⌕<input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="搜索游戏名称"/></label><button>⇩ 批量导入</button><button>⟶ 统一跳转链接</button></div><div className="providerTabs">{providers.map((item)=><button key={item} className={item===provider?"on":""} onClick={()=>setProvider(item)}>{item}</button>)}</div>
